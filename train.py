"""
train.py — Train the seq2seq encoder-decoder chatbot on data.json pairs.
"""
...
pairs: list[tuple[str, str]] = []   # (input_sentence, response_sentence)
...
if len(dataset) == 0:
    print("All samples were filtered out — check your data.json responses.")
    raise SystemExit(1)
...
def teacher_forcing_ratio(epoch: int) -> float:
    start, end = 0.9, 0.3
    return max(end, start - (start - end) * epoch / EPOCHS)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for src, trg in loader:
        ...
        # trg[:, :-1]  → decoder input  (SOS ... last_word), drops EOS
        # trg[:, 1:]   → targets        (first_word ... EOS), drops SOS
        output = model(src, trg[:, :-1], teacher_forcing_ratio=tf_ratio)
        targets = trg[:, 1:]

        # output: (batch, out_len, vocab_size)
        # targets: (batch, trg_len-1)
        # Align lengths — output may be shorter than targets if model stops early
    ...
    if epoch % 50 == 0:
        print(f"  Epoch {epoch:>4}/{EPOCHS}  loss={avg_loss:.4f}  tf={tf_ratio:.2f}")
